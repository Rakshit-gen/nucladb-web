  ...
  return (
    <section className="relative bg-navy-950 py-28 text-white">
      <div className="mx-auto max-w-6xl px-6">
        <Reveal>
          <p className="kicker kicker--on-dark mb-5">Credibility, checked</p>
        ...
        <div className="mt-16 grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 sm:grid-cols-2 lg:grid-cols-3">
          {STATS.map((s, i) => (
            <Reveal key={s.label} delay={i * 0.04} className="bg-navy-950 p-8">
              <p className="font-mono-ui text-[1.15rem] font-medium text-glow-cyan">{s.value}</p>
              <p className="mt-3 text-[0.85rem] leading-relaxed text-white/55">{s.label}</p>
            </Reveal>
          ))}
        </div>
        ...
        <Reveal delay={0.2} className="mt-10">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-8">
            <p className="mb-5 text-[0.9rem] font-semibold text-white/85">What this site does not claim</p>
