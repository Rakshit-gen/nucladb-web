  ...
  return (
    <section className="relative bg-cream py-28">
      <div className="mx-auto max-w-6xl px-6">
        <Reveal>
          <p className="kicker mb-4">Product · Architecture</p>
        ...
        <div className="mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f, i) => (
            <Reveal key={f.title} delay={i * 0.05}>
              <article className="h-full rounded-2xl border border-cream-line bg-white/50 p-6 transition-colors hover:bg-white/90">
                <span className="mb-4 flex h-9 w-9 items-center justify-center rounded-lg bg-navy-900 text-glow-cyan">
                  <f.icon size={17} strokeWidth={1.6} />
                </span>
                <h3 className="text-[0.98rem] font-semibold text-ink">{f.title}</h3>
                <p className="mt-2 text-[0.9rem] leading-relaxed text-ink-soft">{f.body}</p>
              </article>
            </Reveal>
          ))}
        </div>
